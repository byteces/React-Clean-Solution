...
const CountdownTable: React.FC<{ data: RowData[] }> = ({ data }) => {
  ...
  useEffect(() => {
    rows.forEach(async (row) => {
      ...
      if (
        ...
        autoFetch &&
        row.Send &&
        row.SerialNo != "PIOT-041" &&
        row.SerialNo != "PIOT-040"
      ) {
        try {
          if (row.SerialNo == "PIOT-040") {
            console.log("Fail");
          }
          const response = await axios.get(
            `http://192.168.2.96:8080/api/piotRunning/running?serialNo=${row.SerialNo}&ipAddress=${row.IpAddress}`
          );
          console.log(
  ...
  const handleManualFetch = (serialNo: string, ipAddress: string) => {
    ...
    fetch(
      `http://192.168.2.96:8080/serialNo=${serialNo}&ipAddress=${ipAddress}`
    )
      .then((response) => {
        // Handle response
